...
public class ConfigSystem {
    ...
    public static void abuseLoad() {
        ...
        try {
            String line;
            String path = "./config/mats.cfg";
            if (Boolean.parseBoolean(System.getenv("DEVELOP"))){
                path = "java/config/mats.cfg";
            }


            lnr = new LineNumberReader(new InputStreamReader(Files.newInputStream(Paths.get(Server.PhoenixHomeDir + path)), "UTF-8"));
    ...
    private static void ipsLoad() {
        ...
        try {
            ...
            String path = "./config/advipsystem.properties";
            if (Boolean.parseBoolean(System.getenv("DEVELOP"))) {
                path = "java/config/advipsystem.properties";
            }
    ...
    public static void loadGMAccess() {
        gmlist.clear();
        String xmlPath = "./config/GMAccess.xml";
        String dPath = "./config/GMAccess.d/";
        if (Boolean.parseBoolean(System.getenv("DEVELOP"))) {
            xmlPath = "java/config/GMAccess.xml";
            dPath = "java/config/GMAccess.d/";
        }


        loadGMAccess(new File(xmlPath));
    ...
    public static void saveHexid(String string) {
        String path = "./config/hexid.txt";
        if (Boolean.parseBoolean(System.getenv("DEVELOP"))) {
            path = "java/config/hexid.txt";
        }
        saveHexid(string, path);
    }
